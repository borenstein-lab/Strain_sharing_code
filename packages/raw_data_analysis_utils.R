...
reorganize_kraken_table <- function(df) {
  ...
  df %>%
    ...
    mutate(sample_id = str_replace_all(sample_id, "CP04600_L00",  ""))
}
...
# Reorganize dissimilarity tables
reorganize_dissimilarity_table <- function(df) {
  require(dplyr)
  require(tibble)
  
  df %>%
    rename(sample_id = '...1') %>%
    mutate(sample_id = str_replace_all(sample_id, c("CP04600_L00" = "", "\\." = "-", "_B" = ""))) %>%
    rename_with(~ sub("CP04600_L00", '', .) %>% 
                sub("[.]", '-', .)%>% 
                sub("_B", '', .))
}

# Convert diversity table to convenient feature table
reorganize_diversity_table <- function(df) {
  df %>% 
    mutate(sample_id = str_replace_all(sample_id, c("CP04600_L00" = "", "\\." = "-"))) 
}
...
plot_taxonomy <- function(df, rare_otu_cutoff = 0.01, plot_title = 'Taxonomy barplots', legend_title = 'Taxon', n_legend_cols = 1) {
  ...
  # Extract sample's organ
  df = merge(df, metadata)
  df$host_id_unique = str_replace_all(df$sample_id, c("-vaginal" = "", "-rectal" = "", "_1" = ""))
  
  # Sort samples by the abundance of upmost taxon
  df1 = df[df$organ == 'vaginal',]
  ...
  samp_order <- c(samp_order, setdiff(unique(df$host_id_unique), samp_order))
  df$host_id_unique <- factor(df$host_id_unique, levels = samp_order)
  ...
  ggplot(df, aes(fill = otu_grouped, y = relab, x = host_id_unique)) +
